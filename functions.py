...
def initial_conditions_horizontal():
    ...
    dis_x = float(
        input("Enter initial displacement from the equilibrium position [m]: ")
    )
...
def update_vel_euler(force_vec, vel_vec, m, dt):
    ...
    nvel_vec["vel_x"], nvel_vec["vel_y"] = (
        vel_vec["vel_x"] + accel_x * dt,
        vel_vec["vel_y"] + accel_y * dt,
    )
...
def update_dis_euler(dis_vec, vel_vec, dt):
    ...
    ndis_vec["dis_x"], ndis_vec["dis_y"] = (
        dis_vec["dis_x"] + vel_vec["vel_x"] * dt,
        dis_vec["dis_y"] + vel_vec["vel_y"] * dt,
    )
